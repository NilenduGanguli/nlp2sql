...
export const EditorPage: React.FC<EditorPageProps> = ({ initialSql = '', onSqlChange }) => {
  ...
  const [loadedSql, setLoadedSql] = useState(initialSql)
  ...
  const handleRun = useCallback(() => {
    ...
    if (loadedSql) {
      const dist = levenshtein(loadedSql, snapshot)
      if (dist === 0) void emitSignal('ran_unchanged', snapshot, {})
      else void emitSignal('edited_then_ran', snapshot, { edit_distance: dist })
    }
  ...
  }, [executeSql, loadedSql, emitSignal])
